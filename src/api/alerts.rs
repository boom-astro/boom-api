...
pub async fn get_object(
    ...
    let mut brightest_alert_cursor = collection
        .find(doc! {
    ...
    let cursor_alerts = collection
        .find(doc! {
            "objectId": object_id.to_string(),
        })
        .with_options(find_options_alerts)
        .await
        .unwrap();
    let alerts = cursor_alerts
    ...
    let aux_collection: Collection<mongodb::bson::Document> = client
        .database(DB_NAME)
        .collection(aux_collection_name.as_str());
    let aux_entry = aux_collection
        .find_one(doc! {
            "_id": object_id.to_string(),
        })
        .await
        .expect("failed to exexcute find_one query")
        .expect("no aux entry found");
